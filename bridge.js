...
export async function instantDescribeBlocks(attachments, cfg, blocks, prompt, signal) {
  ...
  const results = await Promise.allSettled(
    tasks.map(async ({ id, block }) => {
      let image;
      try {
        // 用完整的图片块引用（attachmentId + mediaType + bytes + width + height）读取，
        // 附件存储会校验引用元数据与文件一致性，缺字段会抛 ATTACHMENT_CORRUPT。
        image = await readAttachmentImage(attachments, block.attachment, combined);
      } catch {
        // 附件读不到：仅当块内真的携带字节内容（Uint8Array/Buffer）时恢复；
        // attachment.bytes 是数字（字节数）时不可用，跳过该图（标记兜底）。
