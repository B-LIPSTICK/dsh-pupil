  ...
  ctx.tools.register(
    defineTool({
      ...
      async execute(args, exec) {
        ...
        // 生成附件立即登记进附件索引：本轮的 vision_* 工具按 attachmentIds
        // 看图时才查得到元数据（预索引只在每轮开始构建，轮中新增的附件覆盖不到）。
        if (ref) {
          indexAttachmentRef({
            ...ref,
            mediaType: ref.mediaType ?? img.mediaType,
            bytes: typeof ref.bytes === "number" ? ref.bytes : img.bytes.length,
          });
        }
        // 2) 同时落盘一份，返回真实文件路径（用户可直接打开 / 模型可用 image_source 兜底）
        let file;
        try {
